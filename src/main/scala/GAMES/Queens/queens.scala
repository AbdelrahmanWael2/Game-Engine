...
//   val rows = new Array[Boolean](8)
  ...
  class queenState extends GameState {
  ...
  def controller(state: GameState): Boolean = {
    ...
    if (
      !(columns.contains(state.input(1))) || !state
        .input(0)
        .isDigit || !(state.input.length == 2)
    )
      return false
    for (i <- 0 to num - 1) {
      if (
        state.input(0) == played(i)(0) || state.input(1) == played(i)(1) || math
          .abs(state.input(0).toInt - played(i)(0))
          .toInt == math.abs(
          columns.indexOf(state.input(1), 0) - columns.indexOf(played(i)(1), 0)
        )
      )
        return false
    }
